...
│   ├── themes.ts                 # Theme tokens — `ThemeColors` interface, `THEMES[]` built-ins, `buildCustomTheme`, `applyThemeCssVars` (sets `--theme-primary/fg`, `--theme-accent`, `--background`, `--foreground`, `--surface`, `--surface-border`, `--text-muted`), `clearThemeCssVars`
...
| GET, DELETE | `/api/admin/blobs` | List / delete R2 objects. `GET` returns `{ bucketName, objectCount, totalSize, blobs[] }` from `listR2BucketObjects()` and enriches user-upload rows from `documents.file_url` + `users` (`documentTitle`, `documentSourceType`, uploader `{ id, name, email }`). `DELETE` calls `deletePdf()`. |
...
| GET | `/api/admin/study-time-chart` | Admin: daily study-time aggregation. Query `?days=N` (default 30, max 90). Returns `{ days: [{ date, totalMinutes, byUser: [{ userId, name, minutes }] }] }` — one entry per calendar day pre-populated with zeros. Only counts completed sessions (`endedAt IS NOT NULL`). Drives the "Total Study Time" stacked bar chart modal in the admin panel. |
...
- **`DocumentPicker.tsx`** — Modes: My Drive, upload (multipart Blob client), textbook catalog; PDF.js outline parsing for chapter ranges; yields `SelectedDocument`. **My Drive:** selecting a saved PDF opens `DocTocEditor` (prefilled from stored `chapterPageRanges` + `pageOffset` when present). **Upload:** after upload, `UploadedDocEditor` uses the same `DocTocEditor`. Shared **`TocEditor`** (`variant="study"`): Visual / JSON / Final tabs; save via `PATCH /api/documents/[id]`.
- **`TocEditor.tsx`** — Shared TOC editor (`variant`: `"admin"` \| `"study"`). Visual + JSON tabs edit *book* pages; Final tab shows `book + offset` (stored PDF ranges). Admin catalog/upload and user upload both use it.
...
- **`ExitGateFlow.tsx`** — Boss Beacons exit gate (20s cooldown → single-boss multi-hit fight → typed phrase fallback); optional fullscreen lock. Replaces legacy exit-password modal. **Only the manual "End Session" button triggers the gate** — fullscreen escape is excluded and falls through to the existing tab-blur / inactivity screen. **`ExitBossFight`** tracks **player focus HP** (100%) and **boss HP**; each wrong answer triggers a **themed boss counter-attack** (persona-specific projectile + damage). If player HP hits 0, a defeat screen appears and the flow continues to the phrase gate (`exit_method: phrase_fallback`).
- **`ExitBossFight.tsx`** — JRPG-style single-boss fight component. The API returns up to 3 MC questions as "hit questions" for the same boss persona; `ExitBossFight` cycles through them internally showing an HP bar (100 → 67 → 33 → 0%), a large boss emoji, and shake/flash/HP-flash animations on each correct hit. The **player sprite** (`PlayerSprite`) is a pure-CSS humanoid (div-based head with hair+eyes, blue-shirt body, skin-tone arms, dark-pant legs with shoes) holding a yellow pencil wand; the wand arm raises via CSS `transition` when `attackPhase !== 'idle'`. Each fight randomly picks one of three **attack styles** (stable for the fight): `"scribble"` — a gold pointed-hexagon bolt with spark accents (`ScribbleBolt`); `"ink"` — a purple radial-gradient sphere (`InkSplash`); `"eraser"` — a pink rectangle with ruled lines (`EraserSmash`). Correct answers sequence through phases `raising → firing → (boss impact) → idle`; wrong answers sequence `raising → blocked → idle` with the projectile bouncing back and the player shaking. All phase transitions and projectile flight are driven by keyframes in `tailwind.config.ts` (`bolt-fly`, `ink-fly`, `eraser-bounce`, `bolt-blocked`, `wand-raise`). Buttons are disabled whenever `attackPhase !== 'idle'`. Two wrong answers on the same hit question trigger the phrase fallback.
...
**Admin (`app/admin/page.tsx`)**
  ...
  - **R2 Storage tab** — Lists every object in the R2 bucket via `listR2BucketObjects()`; header shows bucket name, total bytes in bucket, object count, and usage vs the 10 GB free tier. Each row is tagged with the **R2** badge. User-upload rows show **Uploaded by** from `GET /api/admin/blobs` (`bucketName`, `objectCount`, `totalSize` + enriched blobs). **View** / PDF titles open a preview via **`/api/blob/serve?url=…`** (302 to public r2.dev or presigned URL). Delete confirmation modal stacks above the preview (`z-index`).
...
- **Users tab — "Total Study Time" chart modal.** The "Total Study Time" stat card (top of the Users tab) is clickable. Clicking it opens `StudyTimeChartModal` (`components/admin/StudyTimeChartModal.tsx`) — a full-overlay modal with a **recharts** stacked bar chart showing focused study minutes per day across all users. Each day's bar is split by user (top-8 contributors by total time; remaining users aggregated as "Others"). Range buttons: **7d / 30d / 90d** (default 30). Fetches `GET /api/admin/study-time-chart?days=N` on open and whenever the range changes. Dismiss by clicking ✕ or the backdrop. Dynamically imported with `ssr: false` to avoid recharts SSR issues.
...
- **`/admin/ui-edit`** — Full-screen editor (`requireAdmin()` server gate). **`UiEditShell`** renders page components inside **`UiEditProvider`** (edit mode only; no batch draft). Six tabs: **Home**, **Dashboard**, **Session start**, **Settings**, **Session (live)**, **Boss Beacons**.
...
- **`app/admin/page.tsx`** dynamically imports `StudyTimeChartModal` with `ssr: false` — recharts uses browser APIs that break server pre-rendering.
