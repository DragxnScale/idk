...
2. Query `velocity_question_bank` for rows matching `sourceKey` + `pageIndex IN (reading pages)`, dedupe by question text, shuffle, and pick up to 50. For textbooks, this means a question generated for page 12 of *Zumdahl Chemistry* by user A is automatically available to every user who reads page 12 of that same textbook — no re-generation required.
3. If the bank returns fewer than 50, call the model for **only the shortfall** (with the prompt's target count dynamically adjusted) and persist every new question back to the bank tagged with its `sourceKey` + `pageIndex`. Bank writes are best-effort: a failed insert for one question never blocks the rest or the API response.
4. The final 50 are written to `velocity_games.questions_json` with strict toss-up/bonus alternation enforced by position.
...
2. **Self-contained questions** — no unbound demonstratives or context-dependent pronouns ("this extinction", "the battery", "the passage", "the figure"); every reference must be resolved to a concrete noun so a cold reader can answer.
