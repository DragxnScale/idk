...
| `/api/ai/quiz` | POST, GET | `generateObject` + Zod `quizSchema`. Prompt prioritises in this order: (1) every formula / named equation in the reading (recognition + application), (2) named laws / principles / theories, (3) core definitions and operational vocabulary, (4) cause-and-effect / conditions. Trivia, dates, biographical anecdotes, and side examples are explicitly de-prioritised. Distractors are pulled from nearby/confusable concepts and common misconceptions; the per-question explanation names the formula or law the question hinges on. **After generation runs through `factCheckQuizQuestions` (`lib/ai-fact-check.ts`)**: a verifier sees the questions + source text, applies in-place fixes (wrong correctIndex, generic SA canonical, wrong explanation, bad distractor), and drops anything unsupported by the source. Verifier failures are non-fatal — the unmodified list is returned. | `quizzes` table |
...
| `/api/ai/velocity` | POST, GET | `generateObject` + **flat** Zod schema (OpenAI structured outputs reject `oneOf`, so both MC and SA share one object shape — per-type normalisation happens in TS). Generates in **batches of 12 toss-up/bonus pairs (24 questions)**; POST accepts `continueFromGameId` to append a second batch to the same game (hard cap = 48 total). Topic priority mirrors `/api/ai/quiz`: every formula / named equation pairs as recognition tossup → application bonus; named laws / definitions / cause-and-effect fill the rest; trivia is last-resort. The pre-existing "math goes in BONUS, concept stays in TOSSUP" rule fits this pattern naturally. **After generation runs through `factCheckVelocityQuestions` (`lib/ai-fact-check.ts`)**: same verifier as Quiz, plus pair-atomicity — if either half of a tossup/bonus pair is unfixable, both halves are dropped so the pair structure stays valid. Bank-cached questions skip this pass (they were already verified when first generated). | `velocity_games.questions_json` |
