...
│   │   └── page.tsx              # Fullscreen `/review` queue UI
...
│   ├── review/                   # Spaced-repetition: ReviewSession.tsx fullscreen flow
...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/review/queue` | Build the user's review queue. Joins `flashcards → study_sessions` for ownership and `session_content → documents → textbook_catalog` for the deck title. Returns `{ cards[], queueSize, newRemainingToday, reviewsRemainingToday, capReached }`. Includes Learning / Relearning cards due within the next 10 minutes (so "Again" retries surface in the same session) and orders by `(state-prio, due_at ASC)` so the most-overdue scheduled cards come first. New cards are appended up to the per-user `srs_new_per_day` cap (default 20). The combined output is then truncated to `srs_reviews_per_day` (default 200) so a user returning after a long break gets a manageable pile. |
...
**Page:** [`app/review/page.tsx`](../app/review/page.tsx) → [`components/review/ReviewSession.tsx`](../components/review/ReviewSession.tsx). Fullscreen distraction-free flow — Space flips, 1/2/3/4 grade, on-screen buttons label themselves with the resulting interval (`4d`, `10d`, …) computed client-side via the same `lib/srs.ts` so what the user sees matches what the server actually persists. Relearning cards (Learning / Relearning state after grading) are pushed to the end of the local queue so the user works through everything else first but still re-encounters them in-session; the next refetch picks them up too thanks to the 10-minute lookahead window.
