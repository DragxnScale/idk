...
| `/api/ai/velocity` | POST, GET | `generateObject` + **flat** Zod schema (OpenAI structured outputs reject `oneOf`, so both MC and SA share one object shape — per-type normalisation happens in TS). Generates in **batches of 12 toss-up/bonus pairs (24 questions)**; POST accepts `continueFromGameId` to append a second batch to the same game (hard cap = 48 total). Topic priority mirrors `/api/ai/quiz`: every formula / named equation pairs as recognition tossup → application bonus; named laws / definitions / cause-and-effect fill the rest; trivia is last-resort. The pre-existing "math goes in BONUS, concept stays in TOSSUP" rule fits this pattern naturally. | `velocity_games.questions_json` |
