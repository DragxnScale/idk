...
│   ├── storage-backend.ts        # Pluggable PDF storage adapter (Vercel Blob + Cloudflare R2); R2 single-PUT presign and R2 multipart helpers (`r2StartMultipartUpload`, `r2PresignedUploadPartUrl`, `r2CompleteMultipartUpload`, `r2AbortMultipartUpload`)
